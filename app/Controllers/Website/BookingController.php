...
class BookingController extends BaseController
{
    ...
    public function send()
    {
        ...
        // Push to Internal Lead API
        try {
            $apiUrl = base_url('api/v1/external-lead');
            $apiData = [
                'name'    => $data['full_name'],
                'phone'   => $data['phone'] ?? '',
                'email'   => $data['email'],
                'message' => 'Requested Date: ' . $data['selected_date'] . ' Time: ' . $data['selected_time'] . "\n\n" . ($data['message'] ?? ''),
            ...
            log_message('error', 'Failed to call Lead API: ' . $e->getMessage());
        }
        ...
        // Fix for local XAMPP SSL certificate missing
